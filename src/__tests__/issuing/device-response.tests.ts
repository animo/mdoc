...
import { DataItem, DeviceResponse, Document, MDoc, Verifier, cborDecode, cborEncode, parseDeviceResponse } from '../..'
import {
  DEVICE_JWK,
  ISSUER_CERTIFICATE,
...
describe('issuing a device response', () => {
  ...
  describe('using OID4VPDCAPI handover', () => {
    ...
    it('should match session transcript generated by OpenID Conformance Test Suite', async () => {
      const sessionTranscriptBytes = await DeviceResponse.calculateSessionTranscriptBytesForOID4VPDCApi({
        context: mdocContext,
